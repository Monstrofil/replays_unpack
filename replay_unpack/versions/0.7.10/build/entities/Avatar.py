...
class Avatar(VoiceChatClient, StatsPublisher):
    ...
    # method size: 10000000001
    @unpack_func_args(
        ['GAMEPARAMS_ID', 'UINT32', 'UINT32', 'ENTITY_ID', 'UINT32', 'UINT32', 'UINT32', 'UINT8', 'UINT8', 'INT16',
         ['ARRAY', 'UINT32']])
    def receiveShellInfo(self, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11):
        ...
        with open('record.rec', 'a+') as f:
            res = ''
            res += str(arg1).ljust(15, ' ')
            res += str(arg2).ljust(10, ' ')
            res += str(arg3).ljust(10, ' ')
            res += str(arg4).ljust(5, ' ')
            res += str(arg5).ljust(10, ' ')
            res += str(arg6).ljust(10, ' ')
            res += str(arg7).ljust(10, ' ')
            res += str(arg8).ljust(10, ' ')
            res += str(arg9).ljust(10, ' ')
            res += str(arg10).ljust(10, ' ')
            res += str(arg11).ljust(10, ' ')
            res += '\r'
            f.write(res)
    ...
    @unpack_func_args([['ARRAY', 'SHOT'], ['ARRAY', 'TORPEDOSHOT'], ['ARRAY', 'SHOTKILL'], ['ARRAY', 'EXPLOSION'],
                       ['ARRAY', 'DEPTHCHARGESHOT']])
    def receiveShotPack(self, arg1, arg2, arg3, arg4, arg5):
        self.g_receiveShotPack.fire(self, arg1, arg2, arg3, arg4, arg5)
        for item in arg1:
            with open('record.rec', 'a+') as f:
                f.write('GUN '+str(item) + '\r')
        for item in arg2:
            with open('record.rec', 'a+') as f:
                f.write('TORP '+str(item) + '\r')
        for item in arg3:
            with open('record.rec', 'a+') as f:
                f.write('KILL '+str(item) + '\r')
        for item in arg4:
            with open('record.rec', 'a+') as f:
                f.write('EXPL '+str(item) + '\r')
        for item in arg5:
            with open('record.rec', 'a+') as f:
                f.write(str(item) + '\r')
    ...
    def receiveDamageReport(self, arg1, arg2, arg3):
        ...
        with open('damage.rec', 'a+') as f:
            f.write(str((arg1, arg2, arg3)))
